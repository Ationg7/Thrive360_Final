...
<<<<<<< HEAD
    const loadChallenges = async () => {
      try {
        ...
        if (storedJoined) setJoinedChallenges(JSON.parse(storedJoined));
  ...
  const joinChallenge = (challenge) => {
    setJoinedChallenges((prev) => {
      ...
      const newJoined = [...prev, { ...challenge, status: "In Progress" }];
  ...
  const markDone = (title) => {
    setJoinedChallenges((prevJoined) => {
      const challenge = prevJoined.find((c) => c.title === title);
      if (!challenge) return prevJoined;

      setCompletedChallenges((prevCompleted) => {
        const newCompleted = [
          ...prevCompleted,
          { ...challenge, status: "Completed", completedAt: new Date().toISOString() },
        ];
        localStorage.setItem('completedChallenges', JSON.stringify(newCompleted));
        return newCompleted;
      });

      const updatedJoined = prevJoined.filter((c) => c.title !== title);
